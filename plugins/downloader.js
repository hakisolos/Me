...
*Users Username:* ${igUsername}
*Users Full Name:* ${fullname}
*Users Bio:* ${bio || "NO BIO"}
...
smd(
  ...
  async (m, channelUrl) => {
    try {
      if (!channelUrl) {
        return await m.send("*_Please provide *Queen Nikka a WhatsApp channel URL!_*");
      }
      ...
      if (response.status !== 200 || !response.data.success) {
        return await m.send(
          `*_System Error: ${response.status} ${response.statusText}_*`
        );
      }
      ...
      if (!data) {
        return await m.send("*_nikka did not find any channel information!_*");
      }
...
smd(
    ...
    try {
      ...
      if (!repoUrl) {
        return await _0x1ae8f8.reply(
          "*Provide Repo URL, e.g., .git https://github.com/hakisolos/Queen_Nikka_*"
        );
      }
      ...
      if (!githubRegex.test(repoUrl)) {
        return await _0x1ae8f8.reply("*Provide nikka a valid GitHub Repository URL*");
      }
 ...
 smd({
   ...
   try {
     ...
     if (!_0x204f81) {
       return _0x55aba2.reply("*_Example : .tts Hi,I am Nikka._*");
     }
     try {
 ...
 smd({
   pattern: "sound",
    alias: ["Nikkaai", "aine","mentalism","alive","waso"],
